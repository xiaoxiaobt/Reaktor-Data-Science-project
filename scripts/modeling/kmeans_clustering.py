...
def k_means_clustering(automaticClusterNum=False):
    ...
    def cumulative_variance_plot(array_cum):
        ...
        """
        f, ax = plt.subplots()
        ax.plot(range(len(list(array_cum))), array_cum, '--bo',
                label='PCA explained variance per column')
        leg = ax.legend()
        plt.minorticks_on()
    ...
    columns = list(set(df.columns) -
                   {'Postal code', 'Area', 'text', 'Radius', 'lat', 'lon'})
    ...
    print("Clustering with " + str(i) + " clusters converged in " +
          str(kmeans.n_iter_) + " iterations.")
...
def choose_cluster_num(df=None, start_point=2, end_point=70, rtype='bouldin', plot_inertia=True):
    ...
    :param df: Data to be clustered. Optional. Default: the final data frame returned by 'dataframe()'
    ...
    def clustering_inertia(array_inertia):
        ...
        """
        f, ax = plt.subplots()
        ax.plot(range(2, len(list(array_inertia))+2), array_inertia,
                '--bo', label='Clustering inertia per number of clusters')
        leg = ax.legend()
        plt.minorticks_on()
        plt.grid(b=True, which='major', linestyle='-')
        plt.grid(b=True, which='minor', linestyle=':')
    ...
    for i in k_num:
        ...
        print(
            f'With {i} clusters, Davies Bouldin score is {score_d}, Silhouette score is {score_s}')
        # Save the results
...
if __name__ == '__main__':
    ...
    df.to_csv(Path('dataframes/') / filename,
